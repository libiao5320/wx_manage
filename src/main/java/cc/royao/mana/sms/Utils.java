...
public class Utils {
	...
	private static final List<String> SPECIAL_ENTITIY = Arrays.asList(
			ENTITY_STRING, ENTITY_BYTES, ENTITY_FILE, ENTITY_INPUTSTREAM,
			ENTITY_SERIALIZABLE);
	...
	public static String checkHasParas(String url, List<NameValuePair> nvps,
		...
		if (url.contains("?") && url.indexOf("?") < url.indexOf("=")) {
			Map<String, Object> map = buildParas(url
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public static HttpEntity map2List(List<NameValuePair> nvps,
			...
			for (Entry<String, Object> entry : map.entrySet()) {
				if (SPECIAL_ENTITIY.contains(entry.getKey())) {// 判断是否在之中
					...
					if (ENTITY_STRING.equals(entry.getKey())) {// string
						entity = new StringEntity(String.valueOf(entry
								.getValue()), encoding);
						break;
